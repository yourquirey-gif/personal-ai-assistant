...
function LandingPage({ user, onAuth, onAssistant, onLogout }: { user: User | null; onAuth: (mode: AuthMode) => void; onAssistant: () => void; onLogout: () => void }) {
  return <div className="landing"><header className="site-header landing-header"><Logo /><nav className="desktop-nav"><a href="#features">Features</a><a href="#how-it-works">How it works</a><a href="#use-cases">Use cases</a></nav><div className="header-actions">{user ? <><button className="user-chip" onClick={onAssistant}>{user.picture && <img src={user.picture} alt="" />}{user.name}</button><button className="signin-button" onClick={onLogout}>Sign out</button></> : <><button className="signin-button" onClick={() => onAuth('signin')}>Sign in</button><button className="signup-button" onClick={() => onAuth('signup')}>Sign up</button></>}</div></header><main>
    ...
    <section className="section split-section" id="how-it-works"><div><p className="eyebrow">HOW IT WORKS</p><h2>From a rough question to a useful answer.</h2><p className="section-copy">The assistant helps you move from uncertainty to a clearer plan without making the workflow complicated.</p></div><div className="steps"><div><b>01</b><span><strong>Ask naturally</strong><small>English, Hindi, or Hinglish.</small></span></div><div><b>02</b><span><strong>Explore and understand</strong><small>Get structured explanations and comparisons.</small></span></div><div><b>03</b><span><strong>Take the next step</strong><small>Turn the answer into a practical plan.</small></span></div></div></section>
    <section className="section use-case-section" id="use-cases"><div className="section-heading"><p className="eyebrow">BUILT FOR EVERYDAY WORK</p><h2>Useful across the things that matter.</h2></div><div className="use-case-grid"><div><span>01</span><h3>Study & research</h3><p>Understand chapters, explore topics, and create revision plans.</p></div><div><span>02</span><h3>Career & jobs</h3><p>Research roles, compare paths, and organize requirements.</p></div><div><span>03</span><h3>Everyday planning</h3><p>Break down goals, decisions, errands, and personal projects.</p></div></div></section>
    <section className="final-cta"><p className="eyebrow">START WITH A QUESTION</p><h2>Make your next question useful.</h2><p>Open the assistant and start a focused conversation.</p><button className="primary-cta" onClick={onAssistant}>Open Personal AI <span>→</span></button></section>
  </main><Footer /></div>
}

function AssistantPage({ user, onHome, onLogout }: { user: User; onHome: () => void; onLogout: () => void }) {
  const [input, setInput] = useState(''); const [messages, setMessages] = useState<ChatMessage[]>([]); const [loading, setLoading] = useState(false); const [error, setError] = useState('')
  async function handleSend() { const content = input.trim(); if (!content || loading) return; const next = [...messages, { role: 'user' as const, content }]; setMessages(next); setInput(''); setError(''); setLoading(true); try { const reply = await sendChatMessage(next); setMessages([...next, { role: 'assistant', content: reply }]) } catch (e) { setError(e instanceof Error ? e.message : 'Something went wrong.') } finally { setLoading(false) } }
  return <main className="assistant-shell"><header className="assistant-header"><button className="brand-button" onClick={onHome}><span className="brand-mark">✦</span> Personal AI</button><div className="assistant-user">{user.picture && <img src={user.picture} alt="" />}<span>{user.name}</span><button onClick={onLogout}>Sign out</button></div></header><section className="assistant-main"><div className="assistant-intro"><p className="eyebrow">PERSONAL AI</p><h1>What can I help you with?</h1><p>Research, understand, compare, and plan — all from one conversation.</p></div><div className="conversation">{messages.length === 0 && <div className="empty-chat"><span>✦</span><p>Ask your first question to get started.</p></div>}{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`message ${message.role}`}><span className="message-label">{message.role === 'user' ? 'You' : 'Assistant'}</span><p>{message.content}</p></div>)}{loading && <div className="message assistant"><span className="message-label">Assistant</span><p>Thinking…</p></div>}</div><div className="chat-box"><textarea aria-label="Ask your assistant" placeholder="Ask anything…" rows={3} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void handleSend() } }} /><button type="button" onClick={() => void handleSend()} disabled={loading || !input.trim()}>{loading ? 'Thinking…' : 'Send'}</button></div>{error && <p className="error-message">{error}</p>}</section></main>
}

function App() {
  const [user, setUser] = useState<User | null>(null); const [authMode, setAuthMode] = useState<AuthMode | null>(null); const [path, setPath] = useState(window.location.pathname)
  useEffect(() => { void getCurrentUser().then(setUser) }, [])
  useEffect(() => { const onPop = () => setPath(window.location.pathname); window.addEventListener('popstate', onPop); return () => window.removeEventListener('popstate', onPop) }, [])
  const openAssistant = () => { if (!user) { setAuthMode('signin'); return }; window.history.pushState({}, '', '/assistant'); setPath('/assistant') }
  const home = () => { window.history.pushState({}, '', '/'); setPath('/') }
  const logout = async () => { await signOut(); setUser(null); home() }
  if (path in legalPages) return <LegalPage path={path as LegalPath} />
  if (path === '/assistant' && user) return <AssistantPage user={user} onHome={home} onLogout={logout} />
  return <><LandingPage user={user} onAuth={setAuthMode} onAssistant={openAssistant} onLogout={logout} />{authMode && <AuthModal mode={authMode} onClose={() => setAuthMode(null)} onSwitch={setAuthMode} onSuccess={setUser} />}</>
}
