...
export function assertServerConfig() {
  if (!config.openRouterApiKey) console.warn('OPENROUTER_API_KEY is not configured. Chat requests will return a configuration error.')
  if (!config.mongoUri) console.warn('MONGODB_URI is not configured. MongoDB features will be unavailable.')
  ...
  if (!config.serperApiKey) console.warn('SERPER_API_KEY is not configured. Live Google web research will be unavailable.')
}
