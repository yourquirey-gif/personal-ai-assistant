...
chatRouter.post('/chat', async (req, res) => {
  ...
  try {
    ...
    if (latestUser && conversationObjectId) {
      ...
      await db.collection('messages').insertOne({ conversationId, userId: currentUser.userId, role: 'assistant', content, createdAt: new Date() })
      await db.collection('conversations').updateOne({ _id: conversationObjectId, userId: currentUser.userId }, { $set: { updatedAt: now } })
    }
